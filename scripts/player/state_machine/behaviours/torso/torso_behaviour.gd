...
func check_relevance(input : InputPackage) -> String:	
	# Global falling logic: if behavior is interrupted by fall and player is not on floor, go to midair
	if interrupted_by_fall and not player.is_on_floor():
...
func map_with_dictionary(input : InputPackage, map : Dictionary = behaviour_map):
...
func best_input_inclusive(input : InputPackage) -> String:
	input.behaviour_names.sort_custom(torso.behaviours_priority_sort)
	for behaviour in input.behaviour_names:
		#if resources.can_be_paid(moves_container.moves[action]):
		return behaviour
...
# to work correctly, a behaviour that uses legs double has to have these two methods defined
func setup_legs_animator(_previous_action : LegsAction, _input : InputPackage):
	pass

func update_legs(_input : InputPackage, _delta : float):
	pass
#endregion
...
func accepts_queueing() -> bool:
	return false
	#return backend.get_accepts_queueing(backend_animation, get_progress())

func transitions_to_queued() -> bool:
	return false
	#return backend.get_transitions_to_queued(backend_animation, get_progress())
